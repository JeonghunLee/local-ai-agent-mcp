...
- `docs/` 내부 문서는 제목과 섹션 제목을 영어로 작성한다.
- `docs/` 내부 문서에서는 `Tool`, `Repository`, `Pull Request`, `Issue`, `Review`, `Actions` 같은 기술 용어를 억지로 한글 번역하지 않는다.
- 아키텍처, 설치, 워크플로 문서는 실용적이고 빠르게 훑어볼 수 있게 작성한다.
- `docs/` 내부 문서에서 구조 설명이 필요한 Diagram은 `mermaid`를 사용한다.
- `docs/` 내부 문서 문체는 서술형 설명보다 명사형, 요약형, 표 중심 정리를 우선한다.
- `docs/` 내부 Markdown 문서에서 줄바꿈이 필요한 줄 끝에는 공백 최소 3칸을 남긴다.
...
- [docs/mcp/mcp_server_local.md](docs/mcp/mcp_server_local.md)
