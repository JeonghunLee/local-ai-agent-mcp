...
| **Local (Ollama)** | 실행 + 반복 전담 — Tool 실행 → result.json 확인 → 에러 시 재실행, 여러 TEST 반복으로 문제 패턴 수집 |
...
| Tool | 설명 | 담당 | 실행 |
|------|------|------|------|
| `build_tool()` | make · cmake · bitbake 빌드 실행 | Ollama | 1회 |
| `flash_tool()` | OpenOCD · JLink · dfu-util 플래시 | Ollama | 1회 |
| `do_test_<type>_<nn>()` | 타입별 테스트 실행 — 각자 log 생성 방식 보유 | Ollama | 반복 |
...
| `log_analyzer()` | oops · panic · assert 분석 — 누적 결과 에러 패턴 파악 | Codex | 반복 완료 후 1회 |
| `test_result()` | TEST RESULT 문서 생성 | Codex | 1회 |
...
| Agent | 구분 | Tool | 비고 |
|-------|------|------|------|
| **Ollama** | 실행·반복 | `build_tool` (1회), `flash_tool` (1회), `do_test` (반복) | 빌드·플래시 후 do_test 반복 실행, result.json 확인하며 문제 패턴 수집 |
