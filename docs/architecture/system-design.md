...
- Local AI: Tool 실행 후 `result.json` 저장, 이후 관여 없음
- Sub AI: `result.json`을 읽어 감시, 에러 시에만 개입
...
| Tool | 구분 | 담당 | 실행 |
|------|------|------|------|
| `build_tool()` | Top-level | Local AI | 1회 |
| `flash_tool()` | Top-level | Local AI | 1회 |
| `do_test_<type>_<nn>()` | Top-level | Local AI | 반복 |
...
| `log_analyzer()` | Top-level | Sub AI | 반복 완료 후 1회 |
| `test_result()` | Top-level | Sub AI | 1회 |
...
| Agent | 구분 | Tool | 비고 |
|-------|------|------|------|
| **Local AI** | 실행 | `build_tool`, `flash_tool`, `uart_capture`, `qemu_spawn`, `reg_dump` | 실행만, 분석·판단 없음 |
| Sub AI | 감시·분석 | `log_analyzer`, `test_result` | Local AI 결과 감시 → 에러 시 분석 후 보고 |
