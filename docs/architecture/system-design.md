...
[1.MCP Gateway](../mcp/mcp_gateway.md) — MCP Gateway 라우팅 설정
[2.MCP Local Server](../mcp/local_mcp_server.md) — Local MCP 서버 설정
[3.MCP Github Server](../mcp/github_mcp_server.md) — Github MCP 서버 설정
...
4.2 Main AI API (Cloud)      
    ...
    - **연결 방식**: CLI or `ANTHROPIC_API_KEY` 환경변수   
    - **용도**: 추론, 분석, 장문 컨텍스트 처리


4.3 Sub AI API (Cloud)     
    - **설치 위치**: 없음 — OpenAI 클라우드에서 실행   
    - **연결 방식**: CLI or `OPENAI_API_KEY` 환경변수   
    - **용도**: 코드 생성, 리팩토링, 언어 변환
...
- [mcp/local_mcp_server.md](../mcp/local_mcp_server.md) — Local MCP 서버 설정
